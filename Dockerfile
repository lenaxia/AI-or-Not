...
# Pin npm to match the local toolchain (package-lock.json is generated by
# npm 11.x; node:22 ships npm 10.x, which rejects the lockfile with
# "Missing: esbuild@x.y.z from lock file"). `npm install -g npm@11`
# inside the image bumps to the same version, then `npm ci` works.
