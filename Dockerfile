...
# Use `npm install` (not `npm ci`) so the lockfile reconciles with the
# npm version inside this image (node:22 ships npm 10.x; the lockfile is
# generated by npm 11.x locally, and `npm ci` rejects the difference).
# `npm install` regenerates node_modules from the lockfile intent without
# the strict byte-equality check, then writes back a compatible lockfile.
